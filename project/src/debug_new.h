/*
 * The code below is for debugging purposes, and is included by main.cpp if -DDEBUG flag 
 * is given by compiler
 *
 * NOTE : I didn't written this code, Wu Yongwei did.
 *        check out his 'Nvwa' package @ http://sourceforge.net/projects/nvwa
 *        its dope!
 */
...
#   if !defined(WINDOWSTHREADS) && \
            (defined(WINDOWS) && defined(_MT))
//      Automatically use WINDOWSTHREADS when specifying -MT/-MD in MSVC,
//      or -mthreads in MinGW GCC.
#       define WINDOWSTHREADS
...
# if !defined(_PTHREADS) && !defined(WINDOWSTHREADS) && !defined(_NOTHREADS)
#   define _NOTHREADS
# endif

# if defined(_NOTHREADS)
#   if defined(_PTHREADS) || defined(WINDOWSTHREADS)
#       undef _NOTHREADS
#       error "Cannot define multi-threaded mode with -D_NOTHREADS"
#       if defined(__MINGW32__) && defined(WINDOWSTHREADS) && !defined(_MT)
#           error "Be sure to specify -mthreads with -DWINDOWSTHREADS"
#       endif
#   endif
# endif
...
static bool print_position_from_addr(const void* addr)
{
    ...
    if (new_progname)
    {
        ...
        (defined(WINDOWS) && defined(WINVER) && WINVER >= 0x0500)
...
int check_leaks()
{
    ...
    FILE *new_output_fp = fopen("leaks.txt","wba");
    fprintf( new_output_fp,"\n::: MEMORY LEAKS REPORT :::" );
...
/*
 please make sure the\n"
    ...
    nvwa README file for details (for url see the topcomment of debug.cpp).
*/
#endif
        }
}
